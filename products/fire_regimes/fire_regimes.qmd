...
This product is an integration of different fire metrics derived from the monthly [MapBiomas Fire](https://brasil.mapbiomas.org/en/mapbiomas-fogo/) product in a landscape scale (\~9 km).
...
p <- get_legend_plot("continuous", "Fire Frequency (0-1)", freq_domain, "magma", log=FALSE)
grid.newpage()
grid.draw(p)
```
:::
